var move_h = 0;
var move_v = 0;

var bullet_speed = 1;

if (alive) {
	if (floor(image_index) == Anim_walk) {
	image_index = 0;
	}	
}
dir_chance = max(0, dir_chance - 1);

// Move and handle player logic
if (x == tar_x && y == tar_y) {

    // Change direction
    if (!dir_chance) {
        // Reset delay time
        dir_delay = choose(60, 120, 180);
        dir_chance = dir_delay;
        dir = choose(0, 1, 2, 3);
        
        // Randomly shoot an object
        if (random(1) < 0.5) {  // 50% chance to shoot
            var bullet = instance_create_layer(x, y, "BAIXO", o_slime);
            
            // Set bullet direction based on image_index
            switch (image_index) {
                case 0:
                    bullet.hsp = 0;
                    bullet.vsp = bullet_speed;
                    break;
                case 1:
                    bullet.hsp = -bullet_speed;
                    bullet.vsp = 0;
                    break;
                case 2:
                    bullet.hsp = 0;
                    bullet.vsp = -bullet_speed;
                    break;
                case 3:
                    bullet.hsp = bullet_speed;
                    bullet.vsp = 0;
                    break;
            }
        }
    }

    switch (dir) {
        case 0:
            move_h = 1;
            image_index = 3;
            break;
        case 1:
            move_v = -1;
            image_index = 2;
            break;
        case 2:
            move_h = -1;
            image_index = 1;
            break;
        case 3:
            move_v = 1;
            image_index = 0;
            break;
    }
    
    hsp = 0;
    vsp = 0;

    if (abs(move_h)) {
        tar_x = x + move_h * 16;
        move_v = 0;
    } else if (abs(move_v)) {
        tar_y = y + move_v * 16;
    }

    // Avoid wall
// Avoid wall and void
	if (!place_meeting(tar_x, tar_y, o_Block) && !place_meeting(tar_x, tar_y, o_null)) {
		hsp = move_h * spd;
		vsp = move_v * spd;
	} else {
		// Reset target
		tar_x = x;
		tar_y = y;
	}
} else {
	if (alive) {
		x += hsp;
		y += vsp;
	}
}

