...
if (blink_timer > 0) {
    ...
    for (var i = -2; i <= 2; i++) {
        for (var j = -2; j <= 2; j++) {
            ...
            draw_text(80 + i, 102 + j, string(global.score)); // Desenha o texto com borda preta
        }
    }
    draw_set_color(c_yellow);
    draw_text(80, 102, string(global.score)); // Alterado para desenhar pontos em (16, 16)
    blink_timer--;
} else {
    draw_set_color(c_black); // Define a cor para a borda preta
    for (var i = -2; i <= 2; i++) {
        for (var j = -2; j <= 2; j++) {
            if (abs(i) + abs(j) > 2) continue; // Controla a largura da borda
            draw_text(80 + i, 102 + j, string(global.score) + "X"); // Desenha o texto com borda preta
        }
    }
    draw_set_color(c_white);
    draw_text(80, 102, string(global.score) + "X"); // Alterado para desenhar pontos em (16, 16)
}
